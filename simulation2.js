...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- FIX: Defined base water and stock colors for interpolation ---
    const WATER_COLOR_COMPONENTS = { r: 200, g: 200, b: 255, a: 0.6 };
    const STOCK_COLOR_COMPONENTS = { r: 0, g: 0, b: 255, a: 0.8 };
    const COLORS = {
        // water: `rgba(${WATER_COLOR_COMPONENTS.r}, ${WATER_COLOR_COMPONENTS.g}, ${WATER_COLOR_COMPONENTS.b}, ${WATER_COLOR_COMPONENTS.a})`, // Use getLiquidColor(0) instead
        // stockBlue: `rgba(${STOCK_COLOR_COMPONENTS.r}, ${STOCK_COLOR_COMPONENTS.g}, ${STOCK_COLOR_COMPONENTS.b}, ${STOCK_COLOR_COMPONENTS.a})`, // Use getLiquidColor(STOCK_CONCENTRATION) instead
        glass: '#ddd', label: '#333', /*pipetteFill: 'rgba(0, 0, 255, 0.5)',*/ // Use getLiquidColor for pipette too
    ...
    function initializeState() {
        ...
        highlights = [];
        isDragging = false;
        // --- FIX: Adjusted pipette size, cuvette position, unknown bottle position ---
        const originalPipetteWidth = 10;
        const originalPipetteHeight = 150;
        const pipetteScaleFactor = 0.7; // 30% smaller
        ...
        labObjects = [
            ...
            { id: 'pipette', type: 'pipette', label: 'Pipette', // Label simplified
              x: 50, y: 250,
              width: originalPipetteWidth * pipetteScaleFactor,        // Resized width
              height: originalPipetteHeight * pipetteScaleFactor,      // Resized height
            ...
            { id: 'cuvette', type: 'cuvette', label: 'Cuvette',
              x: 230, y: 320, // Moved 30 right, 30 up from 200, 350
              width: 15, height: 50, maxVolume: 4, currentVolume: 0, concentration: 0, isClean: true, isDraggable: true, isDropTarget: true, isInSpec: false },
            ...
            { id: 'unknownBottle', type: 'bottle', label: 'Unknown Drink',
              x: 50, y: 195, // Moved down 20 more units from 175
              width: 50, height: 80, concentration: -1, maxVolume: 500, currentVolume: 500, isDraggable: false, isDropTarget: true },
        ];
        dataTableData = [ // Concentrations recalculated just in case
        ...
        console.log("State initialized with layout adjustments.");
    }
    ...
    // --- FIX: Modified getLiquidColor to interpolate between water and stock ---
    function getLiquidColor(concentration) {
        if (concentration === null || concentration === undefined) return 'rgba(128,128,128,0.5)'; // Grey for invalid
        if (concentration === -1) return 'rgba(100, 0, 200, 0.7)'; // Purple for unknown

        // Clamp concentration to valid range [0, STOCK_CONCENTRATION] for ratio calculation
        const effectiveConc = Math.max(0, Math.min(concentration, STOCK_CONCENTRATION));
        const ratio = (STOCK_CONCENTRATION > 0) ? effectiveConc / STOCK_CONCENTRATION : 0; // Avoid division by zero

        // Interpolate RGBA components
        const r = Math.round(WATER_COLOR_COMPONENTS.r * (1 - ratio) + STOCK_COLOR_COMPONENTS.r * ratio);
        const g = Math.round(WATER_COLOR_COMPONENTS.g * (1 - ratio) + STOCK_COLOR_COMPONENTS.g * ratio);
        const b = Math.round(WATER_COLOR_COMPONENTS.b * (1 - ratio) + STOCK_COLOR_COMPONENTS.b * ratio); // Blue might not always be 255 if water/stock change
        const a = WATER_COLOR_COMPONENTS.a * (1 - ratio) + STOCK_COLOR_COMPONENTS.a * ratio;
    ...
    // --- FIX: Reworked test tube liquid drawing for better bottom fill ---
    function drawTestTube(obj, isHighlighted) {
        labCtx.save();
        // Draw Tube Body
        labCtx.strokeStyle = '#555';
        labCtx.fillStyle = 'rgba(230, 230, 230, 0.5)'; // Glass effect
        labCtx.fillRect(obj.x, obj.y, obj.width, obj.height); // Rectangular part
        labCtx.strokeRect(obj.x, obj.y, obj.width, obj.height); // Outline rect

        // Draw Tube Bottom (Arc Outline)
        labCtx.beginPath();
        labCtx.arc(obj.x + obj.width / 2, obj.y + obj.height, obj.width / 2, 0, Math.PI);
        labCtx.stroke(); // Outline the bottom arc
        // Fill the bottom arc with glass color
        ...
        if (obj.currentVolume > 0 && obj.maxVolume > 0) {
            ...
            const liquidLevel = obj.height * liquidLevelRatio; // Height from bottom of rect part
            const liquidColor = getLiquidColor(obj.concentration);
            labCtx.fillStyle = liquidColor;

            const tubeRadius = obj.width / 2;
            const liquidTopY = obj.y + obj.height - liquidLevel; // Y-coord of the liquid surface

            // Liquid in Rectangular Part
            ...
            if (arcFillHeight > 0) {
                 // Calculate angle for the arc segment based on fill height
                const angle = Math.acos(1 - arcFillHeight / tubeRadius); // Angle from bottom center
                const startAngle = Math.PI - angle;
                const endAngle = angle; // Corrected end angle relative to Math.PI

                 labCtx.beginPath();
                 // Use arcTo or lineTo + arc to create the segment shape
                 // Move to top-left corner of the arc segment
                 const arcStartX = obj.x + tubeRadius - Math.sin(angle) * tubeRadius;
                 const arcStartY = obj.y + obj.height + Math.cos(angle) * tubeRadius; // Y relative to canvas
                 const arcEndX = obj.x + tubeRadius + Math.sin(angle) * tubeRadius;

                 // Path: Start at top-left liquid corner in arc, draw arc, line to top-right corner
                 labCtx.moveTo(arcStartX, liquidTopY + rectFillHeight); // Approx start
                 labCtx.arc(obj.x + tubeRadius, obj.y + obj.height, tubeRadius, Math.PI - angle, angle); // Draw the bottom arc part
                 labCtx.lineTo(arcEndX, liquidTopY + rectFillHeight); // Close segment top
                 labCtx.closePath();
                 labCtx.fill();

                 // Alternative simpler (less accurate shape) - Fill clipped rectangle
                 /*
                 labCtx.save();
                 labCtx.beginPath();
                 labCtx.arc(obj.x + tubeRadius, obj.y + obj.height, tubeRadius, 0, Math.PI);
                 labCtx.clip(); // Clip subsequent drawing to the arc region
                 // Draw a rectangle that covers the liquid level within the clipped area
                 labCtx.fillRect(obj.x, liquidTopY, obj.width, obj.height + tubeRadius - liquidTopY);
                 labCtx.restore();
                 */
            }
        }
        ...
        // Draw Label
        labCtx.fillStyle = COLORS.label;
        labCtx.textAlign = 'center';
        labCtx.font = '10px sans-serif';
        labCtx.fillText(obj.label, obj.x + obj.width / 2, obj.y + obj.height + 15 + obj.width/2); // Adjust label position slightly
    ...
    // --- FIX: Adjusted pipette drawing for new size ---
    function drawPipette(obj, isHighlighted) {
        ...
        const bulbRadius = 10; // Smaller bulb
        const bulbY = obj.y + bulbRadius;
        const bodyStartY = bulbY + bulbRadius * 0.8; // Start body below bulb center
        const bodyEndY = bodyStartY + obj.height;
        const tipLength = 10; // Shorter tip
        const tipEndY = bodyEndY + tipLength;
        const pipetteTotalHeight = tipEndY - obj.y; // For highlight calculation

        // Bulb
        labCtx.beginPath();
        labCtx.arc(obj.x + obj.width / 2, bulbY, bulbRadius, 0, 2 * Math.PI);
        labCtx.stroke();

        // Body
        labCtx.strokeRect(obj.x, bodyStartY, obj.width, obj.height);

        // Tip
        labCtx.beginPath();
        labCtx.moveTo(obj.x, bodyEndY);
        labCtx.lineTo(obj.x + obj.width / 2, tipEndY);
        labCtx.lineTo(obj.x + obj.width, bodyEndY);
        labCtx.stroke();
        ...
        if (obj.currentVolume > 0) {
            ...
            const liquidStartY = bodyEndY - liquidBodyHeight;
            const color = getLiquidColor(obj.contentsConcentration);
            labCtx.fillStyle = color;

            // Liquid in body
            labCtx.fillRect(obj.x + 1, liquidStartY, obj.width - 2, liquidBodyHeight);

            // Liquid in tip (simplified - assumes tip fills if any liquid is present)
            labCtx.beginPath();
            labCtx.moveTo(obj.x + 1, bodyEndY);
            labCtx.lineTo(obj.x + obj.width / 2, tipEndY);
            labCtx.lineTo(obj.x + obj.width - 1, bodyEndY);
            labCtx.closePath();
            labCtx.fill();
        }

        if (isHighlighted) highlightObjectBorder(obj, 0, 0, obj.width, pipetteTotalHeight); // Use calculated total height
    ...
    function drawGraph() {
        ...
        if (unknownRow && unknownRow.negLogT !== null && KNOWN_SLOPE > 0 && isFinite(unknownRow.negLogT)) {
         ...
         allConcValues.push(STOCK_CONCENTRATION);
        ...
        allAbsValues.push(0.1); // Min abs value for axis scaling
        ...
        graphCtx.lineTo(padding.left, padding.top + plotHeight);
        graphCtx.lineTo(padding.left + plotWidth, padding.top + plotHeight);
        graphCtx.stroke();
        ...
        if (plotData.length > 0 && KNOWN_SLOPE && KNOWN_SLOPE > 0) {
        ...
        if (calculatedUnknownConc !== null && unknownRow && unknownRow.negLogT !== null && isFinite(unknownRow.negLogT)) {
            ...
            const unknownY = scaleY(unknownRow.negLogT);
             if (unknownX >= padding.left && unknownX <= padding.left + plotWidth &&
                ...
                graphCtx.beginPath();
                graphCtx.arc(unknownX, unknownY, 4, 0, 2 * Math.PI);
                graphCtx.fill();
                graphCtx.strokeStyle = 'rgba(0, 128, 0, 0.5)';
                graphCtx.lineWidth = 1;
    ...
    // --- Interaction Logic ---
    // --- FIX: Adjusted isPointOverObject for smaller pipette dimensions ---
    function isPointOverObject(x, y, obj) {
        let hit = x >= obj.x && x <= obj.x + obj.width && y >= obj.y && y <= obj.y + obj.height; // Initial check based on raw x/y/width/height

        if (obj.type === 'pipette') {
            // Define geometry based on drawPipette logic
            const bulbRadius = 10;
            const bulbY = obj.y + bulbRadius;
            const bodyStartY = bulbY + bulbRadius * 0.8;
            const bodyEndY = bodyStartY + obj.height;
            const tipLength = 10;
            const tipEndY = bodyEndY + tipLength;
            const bulbX = obj.x + obj.width / 2;

            // Check Bulb
            const dxBulb = x - bulbX;
            const dyBulb = y - bulbY;
            if (dxBulb * dxBulb + dyBulb * dyBulb <= bulbRadius * bulbRadius) {
                hit = true;
            }
            // Check Body
            else if (x >= obj.x && x <= obj.x + obj.width && y >= bodyStartY && y <= bodyEndY) {
                hit = true;
            }
            // Check Tip (approximate triangle)
            else if (y > bodyEndY && y <= tipEndY) {
                // Simple bounding box for tip for hit detection is often sufficient
                const tipBaseWidth = obj.width;
                const tipXStart = obj.x;
                 if (x >= tipXStart && x <= tipXStart + tipBaseWidth) {
                     hit = true;
                 }
                // More precise triangular check if needed (more complex)
            } else {
                 hit = false; // Explicitly false if not bulb, body, or tip y-range
            }
        } else if (obj.type === 'spectrophotometer') {
             // Check buttons first
             if (isPointInRect(x, y, spec20State.zeroButtonPos)) return { ...obj, clickedButton: 'zero' };
             if (isPointInRect(x, y, spec20State.measureButtonPos)) return { ...obj, clickedButton: 'measure' };
             if (isPointInRect(x, y, spec20State.modeButtonPos)) return { ...obj, clickedButton: 'mode' };

            // Check main body / slot for general hit / drop target
            ...
            if (isPointInRect(x, y, {x: slotX, y: slotY, width: slotWidth, height: slotHeight})) {
                 hit = true; // Hit spec slot
            } else if (x >= obj.x && x <= obj.x + obj.width && y >= obj.y && y <= obj.y + obj.height) {
                hit = true; // Hit main spec body
            } else {
                hit = false;
            }
             return hit ? obj : null; // Return obj if hit, no button distinction needed here for drop target
        } else {
    ...
    function getObjectAt(x, y) { for (let i = labObjects.length - 1; i >= 0; i--) { const obj = labObjects[i]; if (obj === draggedObject) continue; const hitResult = isPointOverObject(x, y, obj); if (hitResult && !hitResult.clickedButton) return hitResult; } return null; } // Exclude button hits for drop targets
    function getObjectAtMouseDown(x, y) { for (let i = labObjects.length - 1; i >= 0; i--) { const obj = labObjects[i]; const hitResult = isPointOverObject(x, y, obj); if (hitResult) return hitResult; } return null; } // Include button hits
    ...
    function updateUI() {
        try {
            ...
            highlights = [];
            if (stepConfig) {
                instructionEl.innerHTML = `<b>Step ${currentStep + 1} / ${instructions.length -1}:</b> ${stepConfig.text}`;
            ...
            feedbackEl.textContent = feedback.message; feedbackEl.className = feedback.type;
             ...
             dataTableData.forEach(row => {
                 ...
                 if (row.id === 'unknown') {
                     if (row.negLogT !== null && KNOWN_SLOPE > 0 && isFinite(row.negLogT)) {
                         displayConc = (row.negLogT / KNOWN_SLOPE).toFixed(3); // Format here
                         displayAbs = parseFloat(row.negLogT).toFixed(4);
                     } else if (row.negLogT === Infinity) {
                         displayConc = 'Too High';
                         displayAbs = `>${MAX_ABS.toFixed(1)}`;
             ...
             } else { slopeDisplayEl.textContent = ''; }
             const unknownRow = dataTableData.find(r => r.id === 'unknown');
             if (unknownRow && unknownRow.negLogT !== null && KNOWN_SLOPE > 0) {
                  if (isFinite(unknownRow.negLogT)) {
                      ...
                      unknownResultEl.innerHTML = `<b>Unknown Drink Conc. ≈ ${calculatedConc.toFixed(3)} µM</b><br><small><i>Calc: Conc = Abs / Slope = ${measuredAbs.toFixed(4)} / ${KNOWN_SLOPE}</i></small>`;
                  } else if (unknownRow.negLogT === Infinity){
                      unknownResultEl.innerHTML = `<b>Unknown Concentration Too High</b><br><small><i>Absorbance > ${MAX_ABS.toFixed(1)}.</i></small>`;
                  } else { unknownResultEl.textContent = ''; }
             } else { unknownResultEl.textContent = ''; }
    ...
    // --- Instructions Definition (Unchanged text, steps refer to smaller pipette implicitly) ---
    const instructions = [
        ...
        { step: 24, text: "Empty the Blank Cuvette: Drag Cuvette from Spec to Waste.", action: 'emptyCuvette', cuvette: 'cuvette', destination: 'wasteBeaker', hint: "Drag cuvette out first, then drag to Waste.", highlight: ['cuvette', 'wasteBeaker'] },
        ...
        { step: 54, id: 'graph_analysis', text: "Calibration complete. Observe Data Table & Graph. Note the slope.", action:'info', highlight: ['data-panel', 'graph-panel'] },
        ...
        { step: 56, text: "Measure Unknown: Fill Pipette (~3mL) from 'Unknown Drink'.", action: 'fillPipette', pipette: 'pipette', source: 'unknownBottle', volume: 3, highlight: ['pipette', 'unknownBottle'] },
        ...
        { step: 58, text: "Set Cuvette as Unknown (Internal Step - Auto).", action: 'setUnknownFlag', cuvette: 'cuvette' },
        ...
        { step: 61, text: "Result recorded. Use Abs and Slope to find concentration.", action:'info', highlight: ['data-panel', 'unknown-result', 'graph-panel'] },
        { step: 62, text: "Experiment Complete! Analysis finished." },
    ];
