...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeState() {
        ...
        // Define bottle positions first to calculate dependent positions
        ...
        // --- FIX: Calculate Pipette position relative to Unknown Drink bottle ---
        const pipetteX = unknownBottle.x + unknownBottle.width + 15; // 15px gap to the right
        const pipetteY = unknownBottle.y; // Align tops
        ...
        // Define tubes to calculate cuvette position
        const tubes = [
        ...
        // --- FIX: Calculate Cuvette position relative to tubes ---
        ...
        const tubeBottomY = firstTube.y + firstTube.height + firstTube.width / 2; // Approximate bottom including arc
        const cuvetteWidth = 15;
        const cuvetteHeight = 50;
        const cuvetteX = tubeRowCenterX - cuvetteWidth / 2; // Center align
        const cuvetteY = tubeBottomY + 30; // 30px gap below tubes

        const cuvette = { id: 'cuvette', type: 'cuvette', label: 'Cuvette',
                          x: cuvetteX, y: cuvetteY,
                          width: cuvetteWidth, height: cuvetteHeight,
                          maxVolume: 4, currentVolume: 0, concentration: 0, isClean: true, isDraggable: true, isDropTarget: true, isInSpec: false };


        labObjects = [
            stockBottle, waterBottle, unknownBottle, // Bottles first
            pipette, // Pipette next
            ...tubes, // Spread the tubes array
            cuvette, // Cuvette positioned relative to tubes
            wasteBeaker, spec20 // Equipment last
        ];
        ...
        spec20State = {
            cuvetteInsideId: null, reading: "-- %T", wavelength: TARGET_WAVELENGTH, isZeroed: false, absorbanceMode: false,
    ...
    function getLiquidColor(concentration) {
        ...
        const ratio = (STOCK_CONCENTRATION > 0) ? effectiveConc / STOCK_CONCENTRATION : 0;
        ...
        const b = Math.round(WATER_COLOR_COMPONENTS.b * (1 - ratio) + STOCK_COLOR_COMPONENTS.b * ratio);
        const a = WATER_COLOR_COMPONENTS.a * (1 - ratio) + STOCK_COLOR_COMPONENTS.a * ratio;
        return `rgba(${r}, ${g}, ${b}, ${a.toFixed(2)})`;
    }

    // --- Drawing Functions (Largely unchanged, pipette/tube drawing updated previously) ---
    ...
    // --- Interaction Logic (Pipette boundary check updated previously) ---
    ...
    // --- FIX: Modified tryEmptyCuvette to check stepConfig for markClean flag ---
    function tryEmptyCuvette(cuvetteId, wasteId, stepConfig) {
        ...
        // Check if this action corresponds to the current step
        const currentStepConfig = instructions[currentStep];
        let stepCompleted = false;
        // Use the passed stepConfig (from mouseup) if available, otherwise use currentStepConfig
        const configToCheck = stepConfig || currentStepConfig;
        ...
        // Set clean status based on the step's configuration
        if (configToCheck?.markClean === true) {
             cuvette.isClean = true;
             console.log("Marking cuvette clean after emptying rinse.");
        } else {
             cuvette.isClean = false; // Default: becomes dirty after emptying sample
             console.log("Marking cuvette dirty after emptying sample.");
        }
    ...
    function updateUI() {
        try {
            ...
            highlights = [];
            // --- FIX: Display total steps correctly ---
            const totalSteps = instructions.length -1; // Exclude final "Experiment Complete" message step
            if (stepConfig) {
                instructionEl.innerHTML = `<b>Step ${currentStep + 1} / ${totalSteps}:</b> ${stepConfig.text}`;
            ...
            feedbackEl.textContent = feedback.message; feedbackEl.className = feedback.type;
             ...
             if (measureCompleteStep > -1 && currentStep >= measureCompleteStep) { slopeDisplayEl.textContent = `Calibration Line Slope (Abs/µM) ≈ ${KNOWN_SLOPE}`; } else { slopeDisplayEl.textContent = ''; }
             const unknownRow = dataTableData.find(r => r.id === 'unknown');
             if (unknownRow && unknownRow.negLogT !== null && KNOWN_SLOPE > 0) { if (isFinite(unknownRow.negLogT)) { const measuredAbs = parseFloat(unknownRow.negLogT); const calculatedConc = (measuredAbs / KNOWN_SLOPE); unknownResultEl.innerHTML = `<b>Unknown Drink Conc. ≈ ${calculatedConc.toFixed(3)} µM</b><br><small><i>Calc: Conc = Abs / Slope = ${measuredAbs.toFixed(4)} / ${KNOWN_SLOPE}</i></small>`; } else if (unknownRow.negLogT === Infinity){ unknownResultEl.innerHTML = `<b>Unknown Concentration Too High</b><br><small><i>Absorbance > ${MAX_ABS.toFixed(1)}.</i></small>`; } else { unknownResultEl.textContent = ''; } } else { unknownResultEl.textContent = ''; }
    ...
    // --- FIX: Added Rinse steps after each emptyCuvette ---
    const instructions = [
        // Sample Prep (Steps 0-19) - Unchanged
        ...
        // Empty Blank + Rinse (Steps 24-27) - NEW Rinse steps added
        ...
        { step: 27, text: "Rinse Cuvette 3/3: Empty rinse water into Waste (Drag Cuvette to Waste).", action: 'emptyCuvette', cuvette: 'cuvette', destination: 'wasteBeaker', markClean: true, hint: "Drag cuvette to Waste.", highlight: ['cuvette', 'wasteBeaker'] },

        // Measure Sample 1 + Rinse (Steps 28-34) - Re-numbered, Rinse steps added
        ...
        { step: 30, text: "Place Cuvette (Sample 1) into Spec.", action: 'insertCuvette', cuvette: 'cuvette', destination: 'spec20', highlight: ['cuvette', 'spec20'] },
        { step: 31, text: "Click the 'Measure' button.", action: 'measure', targetDataRowId: 'tube_10_0', highlight: ['spec20'] },
        { step: 32, text: "Empty Sample 1 Cuvette: Drag Cuvette to Waste.", action: 'emptyCuvette', cuvette: 'cuvette', destination: 'wasteBeaker', markClean: false, highlight: ['cuvette', 'wasteBeaker'] },
        ...
        { step: 35, text: "Rinse Cuvette 3/3: Empty rinse water into Waste.", action: 'emptyCuvette', cuvette: 'cuvette', destination: 'wasteBeaker', markClean: true, highlight: ['cuvette', 'wasteBeaker'] },

        // Measure Sample 2 + Rinse (Steps 36-42) - Re-numbered
        ...
        // Measure Sample 3 + Rinse (Steps 44-50) - Re-numbered
        ...
        // Measure Sample 4 + Rinse (Steps 52-58) - Re-numbered
        ...
        // Measure Sample 5 + Rinse (Steps 60-66) - Re-numbered
        ...
        // Measure Blank again + Rinse (Steps 68-74) - Re-numbered
        ...
        { step: 70, text: "Place Cuvette (Blank) into Spec.", action: 'insertCuvette', cuvette: 'cuvette', destination: 'spec20', highlight: ['cuvette', 'spec20'], allowDirtyInsert: true }, // Allow inserting blank even if marked dirty
        ...
        // Graph Analysis Info Step (Step 76) - Re-numbered
        ...
        // Measure Unknown (Steps 77-81) - Re-numbered
        ...
        { step: 79, text: "Set Cuvette as Unknown (Internal Step - Auto).", action: 'setUnknownFlag', cuvette: 'cuvette' }, // INTERNAL
        ...
        // Final Info Steps (Steps 82-83) - Re-numbered
        ...
        { step: 83, text: "Experiment Complete! Analysis finished." }, // FINAL STEP
    ];
